...
/* A room counts as judged only once it carries a verdict.
 *
 * Correcting a label creates an entry too, and treating that as judged told an
 * annotator a floor was finished when none of its rooms had a verdict -- the
 * collector then kept nothing, because a room without a verdict is not ground
 * truth. Progress has to mean the same thing here as it does in compose.py.
 */
function roomJudged(id) { return !!(S.anno.rooms[id] && S.anno.rooms[id].verdict); }

function storeyDone(st) {
  const r = st.rooms.every((x) => roomJudged(x.id));
...
function renderProgress() {
  ...
  const doneR = st.rooms.filter((r) => roomJudged(r.id)).length;
  ...
  const labelOnly = st.rooms.filter(
    (r) => S.anno.rooms[r.id] && !S.anno.rooms[r.id].verdict).length;
  ...
  $("progress").innerHTML = `
    ...
    (labelOnly ? `<div class="row" style="margin-top:6px;color:var(--v-unsure)">
      <b>${labelOnly} room(s) relabelled but not judged</b></div>
      <div class="muted small">A label is not a verdict — press 1–5 on those or
      they are dropped.</div>` : "");
}
